            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card">
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="datatableRT" class="table table-bordered table-striped">
                                <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Provinsi</th>
                ...
                <div class="modal-content no-r "><a href="#" data-dismiss="modal" aria-label="Close"
                    ...
                    <div
                        ...
                        <div class="text-center p-40 p-b-0" style="margin-bottom: 10px">
                            ...
                            <a href="rt/delete/{{$d->id}}" class="btn btn-primary btn-fab-md" style="width: 80px">Ya</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog width-400" role="document">
                <div class="modal-content no-r "><a href="#" data-dismiss="modal" aria-label="Close"
                    ...
                    <div
                        ...
                        <div class="light p-10 b-t-b">
                            <form action="rt/update/{{$d->id}}" method="post">
                                @csrf
                                <div class="form-group">
                                    <label for="provinsi">Nama Provinsi</label>
                                ...
                                <div class="form-group">
                                    <label for="rw">Nama RW</label>
                                    <select class="form-control" id="rw{{$d->id}}" name="id_rw" required>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="nama">Nama RT</label>
                                    <input id="nama" name="nama" type="text" class="form-control form-control-lg"
                                           placeholder="Nama RT" value="{{$d->nama}}" required>
                                </div>
...
@section('script')
    <script type=text/javascript>
        $(function() {
            $('#datatableRT').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'detail_rw.detail_kelurahan.detail_kecamatan.detail_kab_kota.detail_provinsi.nama',
                        name: 'provinsi'
                    },
                    {
                        data: 'detail_rw.detail_kelurahan.detail_kecamatan.detail_kab_kota.nama',
                        name: 'kabkota'
                    },
                    {
                        data: 'detail_rw.detail_kelurahan.detail_kecamatan.nama',
                        name: 'kecamatan'
                    },
                    {
                        data: 'detail_rw.detail_kelurahan.nama',
                        name: 'kelurahan'
                    },
                    {
                        data: 'detail_rw.nama',
        ...
        $('#kelurahan{{$d->id}}').change(function(){
            var kelurahanID = $(this).val();
            if(kelurahanID){
                $.ajax({
                    type:"GET",
                    ...
                    success:function(res){
                        if(res){
                            $("#rw{{$d->id}}").empty();
                            $("#rw{{$d->id}}").append('<option selected disabled>Pilih RW</option>');
                            $.each(res,function(key,value){
                                $("#rw{{$d->id}}").append('<option value="'+key+'">'+value+'</option>');
                            });
